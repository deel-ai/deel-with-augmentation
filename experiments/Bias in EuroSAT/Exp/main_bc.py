...
from model import ResNet18, Vgg16, Net
...
def train(model, train_loader, cont_loader, confusion_matrix, learning_rate, num_epochs, weight, save_path):
        ...
        criterion = BiasContrastiveLoss(confusion_matrix=confusion_matrix, bb=0)
        # Optimizer Adam
...
def test(model, model_path, test_loader, test_gobal=True):
        ...
        with torch.no_grad():
            ...
            if test_gobal is True:
                ...
                print('Accuracy: {:.4f}%, Error rate of blue images: {:.4f}%'.format(per_crr, err_bl))
            else:
                # Print loss and accuracy
...
def main(num_epochs,
    ...
    for k in range(len(seeds)):
        ...
        confusion_matrix = get_confusion_matrix(num_classes=2, targets=y_train, biases=S_train)
        ...
        X_train_cont_1 = cont_transform(X_train)
        X_train_cont_2 = cont_transform(X_train)
        ...
        X_train_nor = transform(X_train)
        X_test_test = test_transform(X_test)

        train_dataset = TensorDataset(X_train_nor, y_train, S_train)
        ...
        cont_train_dataset = TensorDataset(X_train_cont_1, X_train_cont_2, y_train, S_train)

        weights = [1 / confusion_matrix[c.long(), b.long()] for c, b in zip(y_train, S_train)]
        ...
        new_path = f"{save_folder_path}/checkpoint_bc_seed_{k+1}.pt"
        ...
        train_acc, train_losses = train(model=cnn_model, train_loader=train_dataloader, 
                                        ...
                                        learning_rate=0.001, num_epochs=num_epochs, weight=0.01, save_path=new_path)
...
save_path = '/home/vuong.nguyen/vuong/augmentare/Bias in EuroSAT/Exp/Output_CNN/BC'
