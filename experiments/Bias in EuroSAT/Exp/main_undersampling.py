...
def main(test_set, num_epochs,
    ...
    list_per = [5, 10, 15, 20]
    for per in list_per:
        num_hw = round(len(blue_highway_images)*100/per)
        num_rv = round(len(blue_river_images)*100/per)
        ...
        for k in range(len(seeds)):
            ...
            new_X_train = torch.cat((
                blue_highway_images, blue_river_images,
                highway_images[torch.randint(len(highway_images), (num_hw,))].float(),
                river_images[torch.randint(len(river_images), (num_rv,))].float(), 
            ), 0)

            new_y_train = torch.cat((
                blue_highway_labels, blue_river_labels,
                torch.zeros(num_hw),
                torch.ones(num_rv) 
            ), 0).to(torch.int64)

            new_S_train = torch.cat((
                torch.zeros(len(blue_highway_images)),
                torch.zeros(len(blue_river_images)),
                torch.ones(num_hw),
                torch.ones(num_rv)
            ), 0).to(torch.int64)
...
save_path = '/home/vuong.nguyen/vuong/augmentare/Bias in EuroSAT/Exp/Output_Vgg16/Undersampling'
