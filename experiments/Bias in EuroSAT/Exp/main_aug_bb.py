...
def main(num_epochs,
    ...
    for k in range(len(seeds)):
        ...
        for i in range(len(nb_new_images_1)):
            ...
            new_path = f"{save_folder_path}/checkpoint_aug_bb_seed_{k+1}.pt"

            cnn_model = ResNet18(num_classes=2)
            cnn_model.to(device)
            
            train_acc, train_losses = train(model=cnn_model, train_loader=train_dataloader, 
                                            confusion_matrix=confusion_matrix, learning_rate=0.00008,
...
save_path = '/home/vuong.nguyen/vuong/augmentare/Bias in EuroSAT/Exp/Output_ResNet18/FDA/Aug_BB'
