...
from utils import EarlyStopping, get_confusion_matrix
from bias_balancing import BBLoss
...
def train(model, train_loader, confusion_matrix, learning_rate, num_epochs, save_path):
        # Set loss function with criterion
        criterion = BBLoss(confusion_matrix, device)
        # Optimizer Adam
        ...
        for epoch in range(num_epochs):
            ...
            for i, (images, labels, biases) in enumerate(train_iter):
                ...
                loss = criterion(outputs, labels, biases)
...
def main(num_epochs,
    ...
    for k in range(len(seeds)):
        ...
        new_path = f"{save_folder_path}/checkpoint_bb_seed_{k+1}.pt"

        cnn_model = Vgg16(num_classes=2)
        cnn_model.to(device)
        
        train_acc, train_losses = train(model=cnn_model, train_loader=train_dataloader, 
                                        confusion_matrix=confusion_matrix, learning_rate=0.001,
                                        num_epochs=num_epochs, save_path=new_path)
...
save_path = '/home/vuong.nguyen/vuong/augmentare/Bias in EuroSAT/Exp/Output_Vgg16/BB'
